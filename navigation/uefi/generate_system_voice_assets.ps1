...
function Write-VoiceWav([string]$FileName, [string]$Text, [string]$VoiceName, [int]$Rate) {
  ...
  try {
    ...
    # Clean UEFI voice contract: generate 24 kHz PCM directly. The firmware
    # bank keeps that rate and the HDA renderer performs an exact x2 conversion
    # to 48 kHz, preserving consonant detail without an extra host resample.
