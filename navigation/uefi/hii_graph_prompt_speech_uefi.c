...
/*
 * Read-only runtime browser. It rebuilds the catalogue for exactly one HII
 * form. REF controls can be followed; controls that would invoke callbacks or
 * modify firmware state are never activated by this stage.
 */
        ...
        if (type == 0x02u) {
            ...
            while (q + 2u <= end) {
                ...
                if (op == 0x01u && oplen >= 6u) {
                    ...
                    /*
                     * The decoded BIOS 308 contains two parallel top-level
                     * trees. The M1603QA notebook UI is 0x2716..0x271A.
                     * Filter the legacy/suppressed root before expression
                     * evaluation is added, preventing duplicate hidden tabs.
                     */
