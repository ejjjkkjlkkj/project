...
static int nav_text_is_setup(const char *text, u32 count) {
    return text && count == 5u &&
           text[0] == 's' && text[1] == 'e' && text[2] == 't' &&
           text[3] == 'u' && text[4] == 'p';
}

static int nav_package_is_setup(hii_string_protocol *str, void *handle) {
    if (!str || !handle) return 0;
    ...
    while (p + 4u <= list_end) {
        ...
        if (type == 0x02u) {
            ...
            while (q + 2u <= end) {
                ...
                if (op == 0x0eu && oplen >= 22u) {
                    char title[33];
                    u32 title_count = 0;
                    u16 title_token = rd16(q + 18);
                    if (title_token &&
                        get_hii_string(str, handle, title_token, title, &title_count) &&
                        nav_text_is_setup(title, title_count))
                        return 1;
                }
                q += oplen;
            }
...
/*
 * Runtime read-only form browser. It rebuilds the navigation catalogue for
 * exactly one form, follows EFI_IFR_REF targets with Enter and never writes
 * NVRAM or invokes non-reference actions.
 */
static int nav_load_form(u16 requested_form_id) {
    if (!g_nav_hii_string || !g_nav_hii_handle) return 0;
    u32 list_len = rd32(g_hii_package + 16);
    if (list_len < 24u || list_len > sizeof(g_hii_package)) return 0;
        ...
        if (type == 0x02u) {
            ...
            while (q + 2u <= end) {
                ...
                if (op == 0x01u && oplen >= 6u) {
                    ...
                    if ((requested_form_id == 0u && selected_form_id == 0u) ||
                        ...
                        if (title_token &&
                            get_hii_string(g_nav_hii_string, g_nav_hii_handle,
                                           title_token, g_nav_form_title, &title_count))
                            g_nav_form_title_length = (u8)title_count;
                    }
                } else if (in_selected_form && prompt_opcode(op) && oplen >= 4u) {
                    u16 token = rd16(q + 2);
                    char candidate[33];
    ...
    if (!selected_form_id || !g_nav_prompt_total || g_nav_prompt_overflow) return 0;
...
static int resolve_hii_prompt(void *system_table) {
...
#ifdef QEV_INTERACTIVE_NAV
    /*
     * Bind the exact supplied ASUS M1603QA BIOS 308 package first. The package
     * GUID and root form ID come from the decoded firmware image, not from a
     * visual guess.
     */
    ...
    /*
     * Standards-generic fallback for other firmware: find a FormSet titled
     * Setup and browse its first form read-only.
     */
    g_nav_m1603qa_308_profile = 0u;
    for (u32 hi = 0; hi < handles; ++hi) {
        void *handle = g_hii_handles[hi];
    ...
    /* If no browsable Setup FormSet is found, preserve the flat HII fallback. */
    g_nav_prompt_total = 0;
    g_nav_prompt_index = 0;
    g_nav_prompt_overflow = 0;
    g_nav_help_available = 0;
    ...
    for (;;) {
        ...
        if (st == 0) {
            ...
            if (key.unicode_char == 0x001bu || key.scan_code == 0x0017u) {
                ...
                speech_dma_stop();
                if (g_nav_hii_handle && g_nav_form_history_depth) {
